...
//            'total' => $contractors->total(),
    ...
    public function store(Request $request) {
        ...
        if (!$user || !$user->hasRole('admin')) {
            return response()->json(['error' => 'У вас нет прав для создания контрагента'], 403); // Возвращаем ошибку, если пользователь не администратор
        }
    ...
    public function update(Request $request, string $id) {
        ...
        // Проверяем, имеет ли пользователь роль 'admin'
        if (!$user || !$user->hasRole('admin')) {
            return response()->json(['error' => 'У вас нет прав для обновления контрагента'], 403); // Возвращаем ошибку, если пользователь не администратор
        }
    ...
    public function destroy(string $id) {
        ...
        // Проверяем, имеет ли пользователь роль 'admin'
        if (!$user || !$user->hasRole('admin')) {
            return response()->json(['error' => 'У вас нет прав для удаления контрагента'], 403); // Возвращаем ошибку, если пользователь не администратор
        }
