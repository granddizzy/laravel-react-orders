...
function ProductsList({ loading }) {
  ...
  const isProductInCart = (productId) => cartProducts.some((cartProduct) => cartProduct.id === productId);
    ...
    <>
      ...
      <Box sx={{ border: '1px solid #ccc', borderRadius: 1, overflow: 'hidden' }}>
        ...
        {products.map((product, index) => {
          const isInCart = isProductInCart(product.id); // Проверка товара в корзине
          return (
            <Box
              key={product.id}
              component={Link}
              ...
              sx={{
                ...
                p: 1,
                bgcolor: isInCart
                  ? 'grey.400' // Выделяем цветом, если товар в корзине
                  : index % 2 === 0
                    ? 'grey.50'
                    : 'grey.100',
                '&:hover': {
